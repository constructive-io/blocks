...
- `packages/blocks-schema`, `packages/blocks-renderer`, and
  `packages/json-schema-to-blocks` build with `makage` and publish from `dist`
  (`publishConfig.directory`). They keep root-level entry points and no
  `exports` map, so deep imports stay `blocks-schema/compose`.
