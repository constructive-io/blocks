 ...
 * Auth Error Handling
 *
 * Centralized error code mapping and parsing for auth blocks. Handles GraphQL
 * errors, validation errors (Zod-like), and custom AuthErrors. Ported verbatim
 * from the Constructive admin app (`lib/auth/auth-errors.ts`); self-contained
 * with zero imports so it ships as a standalone `auth-errors` registry item.
 *
