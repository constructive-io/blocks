...
import { getComponentDocNeighbors, type ComponentDocId, type ComponentDocLink } from '@/lib/component-doc-navigation';

function NeighborLink({ direction, neighbor }: { direction: 'Previous' | 'Next'; neighbor?: ComponentDocLink }) {
...
export function ComponentDocPagination({ current }: { current: ComponentDocId }) {
  const { next, previous } = getComponentDocNeighbors(current);

  return (
    <nav aria-label="Component pagination" className="mt-12 grid grid-cols-2 gap-6 border-t border-border pt-6">
