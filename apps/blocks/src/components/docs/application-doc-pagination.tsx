...
function NeighborLink({ direction, neighbor }: { direction: 'Previous' | 'Next'; neighbor?: ApplicationDocLink }) {
  ...
  return (
    <Link
      ...
      href={neighbor.href ?? `/blocks/${neighbor.id}`}
    >
...
export function ApplicationDocPagination({ current }: { current: ApplicationDocId }) {
  const { next, previous } = getApplicationDocNeighbors(current);

  return (
    <nav aria-label="Application block pagination" className="mt-12 grid grid-cols-2 gap-6 border-t border-border pt-6">
