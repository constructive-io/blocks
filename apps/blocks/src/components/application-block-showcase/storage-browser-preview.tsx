...
export function StorageBrowserPreview() {
  ...
  function selectBucket(bucketId: string) {
    ...
    setMessage(`${bucket?.name ?? bucket?.key ?? 'Bucket'} selected.`);
  }
  ...
  return (
    ...
    >
      <StorageBrowser
        ...
        onDropFiles={(files) =>
          setMessage(
            `${files.length} ${files.length === 1 ? 'file' : 'files'} dropped for upload.`,
          )
        }
      />
      ...
      <Sheet open={uploadOpen} onOpenChange={setUploadOpen}>
        <SheetContent className="w-full gap-3 sm:max-w-md" side="right">
          <SheetHeader className="space-y-1 pr-8 pb-0 text-left">
            <SheetTitle className="text-sm font-medium">Upload files</SheetTitle>
            <SheetDescription className="text-[13px]">
              Add objects to {activeBucket?.name ?? activeBucket?.key ?? 'the selected bucket'}.
            </SheetDescription>
          </SheetHeader>
          <UploadDropzone
