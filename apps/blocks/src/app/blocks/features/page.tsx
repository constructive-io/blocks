...
const FEATURE_PACK_INSTALLS = FEATURE_PACK_DOCS.map((pack) =>
  registryAdd(pack.registryName)
).join('\n');
    ...
    <article className="registry-page">
      <header className="mb-10 max-w-3xl">
        ...
        <p className="mt-2 text-pretty text-sm leading-7 text-muted-foreground sm:text-[15px]">
          {DESCRIPTION} Each registry item copies its presentational views,
          provider-neutral contracts, and a machine-readable manifest into your
          project, so the UI and the database preset can describe the same
          capability boundary.
        </p>
      </header>
      ...
      <div className="flex flex-col gap-12 lg:gap-14">
        <section aria-labelledby="feature-pack-model-heading">
          <div className="mb-4 max-w-3xl">
            <h2
              id="feature-pack-model-heading"
              className="text-lg font-semibold tracking-tight"
            >
              The contract between a preset and its UI
            </h2>
            <p className="mt-1.5 text-pretty text-sm leading-7 text-muted-foreground">
              A manifest declares dependencies, endpoint kinds, required and
              optional capabilities, and the metadata sections a feature needs.
              Adapters bind those contracts to live resources and actions, while
              PostgreSQL grants and RLS remain the authorization boundary for
              every request.
            </p>
          </div>
        ...
        <section aria-labelledby="feature-pack-catalog-heading">
          <div className="mb-4 max-w-3xl">
            <h2
              id="feature-pack-catalog-heading"
              className="text-lg font-semibold tracking-tight"
            >
              First-release catalog
            </h2>
            <p className="mt-1.5 text-pretty text-sm leading-7 text-muted-foreground">
              Install one pack when you are composing a focused surface. The
              registry resolves its declared pack dependencies automatically.
            </p>
          </div>
        ...
        <section aria-labelledby="preset-profile-heading">
          <div className="mb-4 max-w-3xl">
            ...
            <p className="mt-1.5 text-pretty text-sm leading-7 text-muted-foreground">
              These profiles map the current backend preset slugs to the feature
              packs they are expected to support. Treat them as compatibility
              declarations while backend preset behavior is still being
              stabilized; installing a profile copies its manifest and its
              transitive UI packs, but it does not provision or migrate a
              database.
            </p>
          </div>
