    ...
    <article className="registry-page">
      <header className="mb-10 max-w-3xl">
        ...
        <p className="mt-2 text-pretty text-sm leading-7 text-muted-foreground sm:text-[15px]">
          {DESCRIPTION} Pass the secret-free tenant descriptor returned by provisioning; the first-party wrapper handles
          standalone auth, capability discovery, endpoint-scoped requests, and pack adapters.
        </p>
      </header>
      ...
      <div className="flex flex-col gap-12 lg:gap-14">
        <section aria-labelledby="console-showcase-heading">
          <div className="mb-4 max-w-3xl">
            <h2 id="console-showcase-heading" className="text-lg font-semibold tracking-tight">
              Product states
            </h2>
            <p className="mt-1.5 text-pretty text-sm leading-7 text-muted-foreground">
              Console Kit is a tenant-user console. Switch presets and signed-out, discovering, partial, ready,
              incompatible, and unavailable states without a live fixture. Missing public routes are expected degraded
        ...
        <section aria-labelledby="console-install-matrix-heading">
          <div className="mb-4 max-w-3xl">
            <h2 id="console-install-matrix-heading" className="text-lg font-semibold tracking-tight">
              Choose an install surface
            </h2>
            <p className="mt-1.5 text-pretty text-sm leading-7 text-muted-foreground">
              One matrix covers the umbrella, core, Console modules, standalone packs, and official presets. Installed
              code never grants authority; each surface only becomes interactive when public evidence supports it.
            </p>
          </div>
          <Table
          ...
          >
            <TableCaption className="sr-only">
              Install surfaces, what they install, expected public evidence, and degradation behavior.
            </TableCaption>
            <TableHeader>
              <TableRow>
            ...
            <TableBody>
              {INSTALL_MATRIX.map((row) => (
                <TableRow key={row.surface}>
                  <TableCell className="font-mono text-xs font-medium whitespace-nowrap">{row.surface}</TableCell>
        ...
        <section aria-labelledby="console-install-heading">
          <div className="mb-4 max-w-3xl">
            <h2 id="console-install-heading" className="text-lg font-semibold tracking-tight">
              Install the full console
            </h2>
            <p className="mt-1.5 text-pretty text-sm leading-7 text-muted-foreground">
              One registry item installs the Console Kit, all seven feature modules, the runtime and catalog contracts,
              the single modular Zustand store, and the shadcn Base UI app shell dependency graph.
            </p>
          </div>
          <CodeBlock label="shadcn CLI">{registryAdd('console-kit-nextjs')}</CodeBlock>

          <div className="mt-8 max-w-3xl">
            <h3 className="text-sm font-medium text-foreground">Install an official preset</h3>
            <p className="mt-1.5 text-pretty text-sm leading-7 text-muted-foreground">
              Each preset registry item installs the core and the exact feature modules mapped to its Constructive DB
              preset. Runtime navigation still follows the contracts discovered from the active tenant, so installed
              code never grants a capability by itself.
            </p>
          </div>
          <div className="mt-3 grid gap-3 lg:grid-cols-3">
            {[
              ...
              <div key={preset.registryName} className="min-w-0 rounded-xl border border-border/60 bg-card p-4">
                <h4 className="text-sm font-medium text-foreground">{preset.title}</h4>
                <p className="mt-1 text-xs leading-5 text-muted-foreground">{preset.modules}</p>
          ...
          <div className="mt-8 max-w-3xl">
            <h3 className="text-sm font-medium text-foreground">Compose feature modules independently</h3>
            <p className="mt-1.5 text-pretty text-sm leading-7 text-muted-foreground">
              Install the leaf-independent core and only the Console Kit modules your product needs. Every console
              module installs its standalone, provider-neutral feature-pack view transitively, then adds discovery,
              navigation, its Constructive adapter when available, and any pack-owned store slice. Install a
              feature-pack item alone when the host supplies the view props without Console Kit.
            </p>
          </div>
          ...
          <CodeBlock className="mt-4" label="Selected-pack Constructive composition" language="tsx">
            {LOWER_LEVEL_EXAMPLE}
          </CodeBlock>
        </section>
        ...
        <section aria-labelledby="console-host-contract-heading">
          <div className="mb-4 max-w-3xl">
            <h2 id="console-host-contract-heading" className="text-lg font-semibold tracking-tight">
              Start from the tenant endpoint contract
            </h2>
            <p className="mt-1.5 text-pretty text-sm leading-7 text-muted-foreground">
              The host supplies a database ID and its routable public GraphQL endpoints; Console Kit does not need a
              preset name or a control-plane receipt at runtime. Keep credentials on the user side of the sign-in form,
              because the secret-free descriptor is safe to pass from a server component to the client. A host-owned
              session must carry the same database ID, which prevents identity state from crossing between tenants.
            </p>
          </div>
          ...
          <div className="grid gap-2 md:grid-cols-3">
            {[
              ...
              <div key={boundary.title} className="rounded-xl border border-border/60 bg-card p-4">
                <div className="flex flex-wrap items-center justify-between gap-2">
                  <h3 className="text-sm font-medium text-foreground">{boundary.title}</h3>
                  <Badge variant="outline">{boundary.badge}</Badge>
                </div>
                <p className="mt-2 text-pretty text-xs leading-5 text-muted-foreground">{boundary.body}</p>
              </div>
            ))}
          </div>

          <CodeBlock className="mt-4" label="app/console/application-console.tsx" language="tsx">
            {CONSOLE_EXAMPLE}
          </CodeBlock>

          <div className="mt-4 max-w-3xl">
            <p className="text-pretty text-sm leading-7 text-muted-foreground">
              Use the lower-level Console Kit endpoint resolver for embedded or non-Constructive providers. Endpoint
              fallback stays opt-in, so an admin operation cannot silently cross onto the data endpoint.
            </p>
          ...
          <div className="mt-6 max-w-3xl">
            <h3 className="text-sm font-medium text-foreground">Bootstrap CSRF for hardened auth</h3>
            <p className="mt-1.5 text-pretty text-sm leading-7 text-muted-foreground">
              When a tenant enables{' '}
              <code className="rounded bg-muted px-1 py-0.5 font-mono text-[12px]">require_csrf_for_auth</code>, pass an
              async token provider. Its host endpoint creates a short-lived anonymous session with a cryptographically
              random secret through a trusted backend connection and returns that secret once; Console Kit sends it only
              as the auth mutation&apos;s{' '}
              <code className="rounded bg-muted px-1 py-0.5 font-mono text-[12px]">csrfToken</code>. Constructive
              consumes the anonymous session after successful authentication, so the provider must mint a fresh token
              for each attempt and must never cache it in browser storage.
            </p>
            <CodeBlock className="mt-3" label="Hardened auth token provider" language="tsx">
          ...
          <div className="mt-6 max-w-3xl">
            ...
            <p className="mt-1.5 text-pretty text-sm leading-7 text-muted-foreground">
              Unverified signed-in accounts can send a fresh verification email from Account security. Your verification
              route reads the <code className="rounded bg-muted px-1 py-0.5 font-mono text-[12px]">email_id</code> and{' '}
              <code className="rounded bg-muted px-1 py-0.5 font-mono text-[12px]">verification_token</code> values from
              a URL fragment. When <code className="rounded bg-muted px-1 py-0.5 font-mono text-[12px]">callback</code>{' '}
              is omitted, Console Kit captures those credentials into its closure-owned vault, scrubs the fragment, and
              then the auth adapter calls{' '}
              <code className="rounded bg-muted px-1 py-0.5 font-mono text-[12px]">verifyEmail</code> using the freshly
              delivered credential, including from a fresh signed-out browser. After sign-in, Console Kit reloads the
              account so the verified state is authoritative. Treat the token as a credential: route these values
              through the URL&apos;s <code className="rounded bg-muted px-1 py-0.5 font-mono text-[12px]">#</code>{' '}
              fragment so they stay out of the HTTP request. Pass{' '}
              <code className="rounded bg-muted px-1 py-0.5 font-mono text-[12px]">callback={'{false}'}</code> only when
              the host owns the complete callback lifecycle.
            </p>
            <CodeBlock className="mt-3" label="app/verify-email/verification-console.tsx" language="tsx">
        ...
        <section aria-labelledby="console-store-heading">
          <div className="mb-4 max-w-3xl">
            <h2 id="console-store-heading" className="text-lg font-semibold tracking-tight">
              One store, modular slices
            </h2>
            <p className="mt-1.5 text-pretty text-sm leading-7 text-muted-foreground">
              Each Console Kit instance creates one Zustand vanilla store and composes the core navigation, tenant,
              session, endpoint, discovery, runtime, and adapter slices with slices contributed by installed feature
              modules. The store is scoped to the mounted console, so server renders and multiple consoles cannot leak
              identity or navigation state across instances. If the host owns the store, it must compose the same module
              slices; Console Kit verifies that composition and resets its scoped state when the tenant or identity
              changes.
            </p>
          </div>
        ...
        <section aria-labelledby="console-meta-heading">
          <div className="mb-4 max-w-3xl">
            <div className="flex flex-wrap items-center gap-2">
              <h2 id="console-meta-heading" className="text-lg font-semibold tracking-tight">
                Metadata is a compatibility gate
              </h2>
              <Badge variant="info">_meta 2026-07</Badge>
            </div>
            <p className="mt-1.5 text-pretty text-sm leading-7 text-muted-foreground">
              The runtime checks every configured, reachable endpoint with standard GraphQL introspection and
              Constructive&apos;s versioned{' '}
              <code className="rounded bg-muted px-1 py-0.5 font-mono text-[12px]">_meta</code> contract. This release
              accepts contract version{' '}
              <code className="rounded bg-muted px-1 py-0.5 font-mono text-[12px]">2026-07</code> only; each feature
              module selects its evidence from the endpoint map, and incompatible metadata stops that feature instead of
            ...
            <ol className="grid gap-4 md:grid-cols-3">
              {[
                ...
                <li key={step.title} className="min-w-0">
                  ...
                  <h3 className="mt-1.5 text-sm font-medium text-foreground">{step.title}</h3>
                  <p className="mt-1 text-pretty text-xs leading-5 text-muted-foreground">{step.body}</p>
                </li>
              ))}
            </ol>
          </div>
        ...
        <section aria-labelledby="console-native-proof-heading">
          <div className="mb-4 max-w-3xl">
            <h2 id="console-native-proof-heading" className="text-lg font-semibold tracking-tight">
              Prove compatibility against native tenants
            </h2>
            <p className="mt-1.5 text-pretty text-sm leading-7 text-muted-foreground">
              Start the untouched Constructive DB services with{' '}
              <code className="rounded bg-muted px-1 py-0.5 font-mono text-[12px]">fun up</code>, then let the Blocks
              fixture call the native provisioning procedure for the three official presets and one supported
              storage-routed tenant. The generated secret-free manifest feeds the integration route, while the live
              suite exercises auth, identity-scoped CRUD, cross-tenant rejection, capability discovery, and RLS through
              public GraphQL endpoints. Cleanup uses only the exact tenant database IDs recorded by that run.
            </p>
          </div>
          <CodeBlock label="Native Console Kit proof">{NATIVE_PROOF_EXAMPLE}</CodeBlock>
        </section>

        <section aria-labelledby="console-shell-heading">
          <div className="mb-4 max-w-3xl">
            <h2 id="console-shell-heading" className="text-lg font-semibold tracking-tight">
            ...
            <p className="mt-2 text-pretty text-sm leading-7 text-muted-foreground">
              ...
              <code className="rounded bg-muted px-1 py-0.5 font-mono text-[12px]">defaultRoute</code>. For URL-owned
              navigation, parse the current URL into a typed semantic route and provide the route, href encoder, change
              callback, and link renderer together.
            </p>
          </div>
          ...
          <div className="grid gap-3 md:grid-cols-[minmax(0,1fr)_minmax(0,1fr)]">
            ...
            <div className="rounded-xl border border-border/60 bg-card p-4">
              ...
              <p className="mt-1 text-pretty text-xs leading-5 text-muted-foreground">
                Use the shell directly for a custom application, install a focused pack for one workflow, or start with
                Console Kit when you want the complete discovery and navigation layer.
              </p>
              <Link
                href="/blocks/features"
                className="mt-3 inline-flex min-h-10 items-center text-sm font-medium text-link outline-none underline-offset-4 hover:underline focus-visible:ring-[3px] focus-visible:ring-ring/50"
              >
