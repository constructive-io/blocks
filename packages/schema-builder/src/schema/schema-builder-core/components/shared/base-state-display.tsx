 ...
 * Shows AuthErrorBanner for authentication errors, regular error banner otherwise
 */
...
export function NoDatabaseBanner({
	...
	const { onNavigate } = useSchemaBuilderRuntime();
	const action = onNavigate ? (
		<Button
			variant='outline'
			size='sm'
			className='h-8 px-3 text-xs shrink-0'
			onClick={() => onNavigate({ href: linkHref })}
		>
			{linkText}
		</Button>
	) : (
