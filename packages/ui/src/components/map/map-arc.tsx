...
type MapArcProps<T extends MapArcDatum = MapArcDatum> = {
   ...
   * defaults (the semantic map accent token, `line-width: 2`, and
   * `line-opacity: 0.85`).
...
const DEFAULT_ARC_OPACITY = 0.85;
...
function MapArc<T extends MapArcDatum = MapArcDatum>({
  ...
  const defaultPaint = useMemo<MapArcLinePaint>(
    () => ({
      'line-color': resolveMapColor(map, 'accent', resolvedTheme),
      'line-width': DEFAULT_ARC_WIDTH,
      'line-opacity': DEFAULT_ARC_OPACITY,
    }),
    [map, resolvedTheme],
  );
  const mergedPaint = useMemo(
    () => mergeHoverPaint({ ...defaultPaint, ...paint }, hoverPaint),
    [defaultPaint, paint, hoverPaint],
  );
  const mergedLayout = useMemo(() => ({ ...DEFAULT_ARC_LAYOUT, ...layout }), [layout]);

  const hitWidth = useMemo(() => {
    const w = paint?.['line-width'] ?? DEFAULT_ARC_WIDTH;
