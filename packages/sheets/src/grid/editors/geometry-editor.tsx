...
import { AlertCircle, FileText, Map, MapPin, Shuffle } from 'lucide-react';
...
import { SheetsContext } from '../../context/sheets-context';
import type { MapPickerValue } from '../../utils/map-picker';
import { EditorFocusTrap } from './editor-focus-trap';
import { GEOJSON_GEOMETRY_TYPES } from './geometry-types';
import { OVERLAY } from './overlay-presets';
import { OverlayMeasureContext } from './overlay-viewport-guard';

// Lazy-load MapPicker so MapLibre stays out of the main Sheets chunk.
const MapPicker = lazy(() => import('../../utils/map-picker').then((m) => ({ default: m.MapPicker })));

function MapPickerLoading() {
  return (
    <div className="relative h-[280px] w-full overflow-hidden rounded-lg border" role="status">
...
interface MapPickerErrorBoundaryProps {
  children: ReactNode;
  onError: (error: unknown) => void;
  onFallback: () => void;
}

class MapPickerErrorBoundary extends Component<MapPickerErrorBoundaryProps, { error: unknown }> {
  ...
  render(): ReactNode {
    ...
    return (
      <div
        className="flex min-h-[280px] items-center justify-center rounded-lg border bg-muted/30 p-4 text-sm text-muted-foreground"
        role="alert"
      >
...
function isValidGeoJSON(obj: any): boolean {
  ...
  if (!(GEOJSON_GEOMETRY_TYPES as readonly string[]).includes(obj.type)) return false;
...
function getGeoJSONHint(obj: any): string | undefined {
  ...
  if (obj.type === 'FeatureCollection')
    return 'FeatureCollection type is not supported by PostGIS. Extract individual geometries first.';
  return undefined;
}
...
const GEOMETRY_PLACEHOLDERS: Record<string, string> = {
  ...
  MultiLineString: JSON.stringify(
    {
...
function generateRandomGeometry(constrainedType?: string): GeometryType {
  ...
  const randomType =
    constrainedType && RANDOMIZABLE_TYPES.includes(constrainedType)
      ? constrainedType
  ...
  switch (randomType) {
    ...
    case 'LineString': {
      const pathPoints = [];
      for (let i = 0; i < 4; i++) {
        pathPoints.push([baseLng + (Math.random() - 0.5) * 0.01, baseLat + (Math.random() - 0.5) * 0.01]);
      }
      geojson = {
        type: 'LineString',
...
export const GeometryEditor: React.FC<GeometryEditorProps> = ({ value, onFinishedEditing, expectedType }) => {
  const config = useContext(SheetsContext)?.config;
  ...
  const canShowMap =
    expectedType === 'Point' || (!expectedType && geometryValue != null && isPointGeometry(geometryValue));
  ...
  const handleMapError = useCallback(
    (error: unknown) => config?.onError?.(error, { source: 'editor' }),
    [config?.onError],
  );
  const handleMapFallback = useCallback(() => setActiveTab('json'), []);
    ...
    <div className="flex items-center justify-between border-t px-3 py-2">
      <div className="text-muted-foreground flex items-center gap-3 text-xs">
        ...
        <span className="inline-flex shrink-0 items-center gap-1">
          <kbd className="bg-muted/60 border-border/40 inline-flex min-h-5 min-w-5 items-center justify-center rounded border font-sans text-[11px] leading-tight">
            {MOD_KEY}
          </kbd>
          <kbd className="bg-muted/60 border-border/40 inline-flex min-h-5 min-w-5 items-center justify-center rounded border font-sans text-[11px] leading-tight">
            ↵
        ...
        <span className="inline-flex shrink-0 items-center gap-1">
          <kbd className="bg-muted/60 border-border/40 inline-flex min-h-5 items-center justify-center rounded border px-1.5 font-sans text-[11px] leading-tight">
            Esc
          </kbd>
          <span>cancel</span>
        </span>
    ...
    >
      <div ref={editorRef} onKeyDown={handleEditorKeyDown} className="flex min-h-0 flex-1 flex-col">
        {canShowMap ? (
          <Tabs
            value={activeTab}
            onValueChange={(v) => setActiveTab(v as 'map' | 'json')}
          ...
          >
            ...
            <div
              ...
              style={{ maxHeight: scrollBudget }}
            >
              <TabsContent value="map" className="mt-0 p-3">
                <MapPickerErrorBoundary onError={handleMapError} onFallback={handleMapFallback}>
